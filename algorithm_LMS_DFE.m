function [overall_squared_error_seq, overall_pred_signal] = algorithm_LMS_DFE(Params, known_train_seq, full_noised_signal_seq)
    ...
    ff_filter_coeffs = zeros(1, Params.LMS_DFE.Lfff);
    fb_filter_coeffs = zeros(1, Params.LMS_DFE.Lfbf);
    %{
    ...
    for repetition = 1:Params.NumRepetition
        ...
        [error_seq, ff_filter_coeffs, fb_filter_coeffs] = ...
            LMS_DFE_train(Params, ff_filter_coeffs, fb_filter_coeffs, known_train_seq, ...
            full_noised_signal_seq(1,start_index:-1+start_index+Params.train_length+Params.LMS_DFE.Lfff));
        squared_error_seq = error_seq .^ 2;
        
        % Prepare data for decision-directed phase
        noised_signal = zeros(1,Params.data_length+Params.LMS_DFE.Lfff);
        ...
        noised_signal(1,Params.data_length:-1+Params.data_length+Params.LMS_DFE.Lfff) = ...
            full_noised_signal_seq(1,start_index:-1+start_index+Params.LMS_DFE.Lfff);

        % Prediction/inference/decision-directed phase
        pred_signal = LMS_DFE_inference(ff_filter_coeffs, fb_filter_coeffs, noised_signal);
...
function [error_seq, ff_filter_coeffs, fb_filter_coeffs] = LMS_DFE_train(Params, ff_filter_coeffs, fb_filter_coeffs, known_train_seq, full_noised_signal_seq)
    error_seq = zeros(1, Params.train_length);
    
    ff_input = zeros(1, Params.LMS_DFE.Lfff);
    fb_input = zeros(1, Params.LMS_DFE.Lfbf);
    fb_pred = 0;
    ...
    for iteration = 1:Params.train_length
        ...
        ff_filter_coeffs = ff_filter_coeffs + ...
            Params.LMS_DFE.alpha*error_seq(iteration).*ff_input;
        fb_filter_coeffs = fb_filter_coeffs - ...
            Params.LMS_DFE.alpha*error_seq(iteration).*fb_input;
...
function pred_signal = LMS_DFE_inference(ff_filter_coeffs, fb_filter_coeffs, noised_signal)
